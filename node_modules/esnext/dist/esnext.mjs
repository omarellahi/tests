...
import { isNode, NODE_FIELDS, isProperty, isClassMethod, isNewExpression, isReturnStatement, isExpressionStatement, isMemberExpression, isFunctionExpression, isIdentifier, isThisExpression, isSequenceExpression, isArrowFunctionExpression, isCallExpression, isBinaryExpression, isLogicalExpression, isAssignmentExpression, isProgram, isFunction, isObjectExpression, isObjectProperty, exportNamedDeclaration, exportSpecifier, identifier, exportAllDeclaration, exportDefaultDeclaration, variableDeclaration, variableDeclarator, isVariableDeclaration, importDeclaration, importDefaultSpecifier, importSpecifier, isObjectPattern, isStringLiteral, isUnaryExpression, isSwitchCase, isVariableDeclarator, objectPattern, objectProperty, assignmentExpression, stringLiteral, templateElement, templateLiteral, objectMethod } from '@babel/types';
...
function visitor(module) {
  ...
  return {
    FunctionExpression(path) {
      ...
      if (isExpressionStatement(parent) && hasParens(path, module)) {
...
function visitor$1(module) {
  ...
  return {
    Program(path) {
      ...
      rewriteImportsAndExports(path, module, options.safeFunctionIdentifiers, options.forceDefaultExport);
    },
...
function rewriteImportsAndExports(path, module) {
  ...
  var firstUnsafeLocation = getFirstUnsafeLocation(path, ['require'].concat(toConsumableArray(safeFunctionIdentifiers)));
  body.forEach(function (statement) {
...
function isExportsObject(path) {
  ...
  if (isMemberExpression(node)) {
    return !path.scope.hasBinding('module') && isIdentifier(node.object, { name: 'module' }) && isIdentifier(node.property, { name: 'exports' });
...
function rewriteSingleExportAsDefaultExport(path, module) {
  ...
  if (isSimpleObjectExpression(right)) {
    ...
    path.replaceWith(exportNamedDeclaration(null, _bindings.map(function (binding) {
    ...
    if (_pathNode) {
      ...
      path.replaceWith(exportAllDeclaration(_pathNode));
    } else {
...
function rewriteAssignmentToDefaultExport(path, module) {
  var node = path.node;
  var right = path.node.expression.right;
  metadata$1(module).exports.push({ type: 'default-export', node: cleanNode(node) });

  module.magicString.overwrite(path.node.start, getAssignmentEqualsEnd(node, module), 'export default ');

  path.replaceWith(exportDefaultDeclaration(right));
}
...
function rewriteNamedFunctionExpressionExport(path, module) {
  ...
  if (localName === exportName) {
    ...
    path.replaceWith(exportNamedDeclaration(right, [], null));
  } else {
    ...
    path.replaceWithMultiple([declaration, exportNamedDeclaration(null, [exportSpecifier(localId, identifier(exportName))])]);
  }
}
...
function rewriteNamedValueExport(path, module) {
  ...
  if (localBinding === property.name) {
    ...
    path.replaceWith(exportNamedDeclaration(variableDeclaration('let', [variableDeclarator(identifier(property.name), right)]), [], null));
  } else {
    ...
    path.replaceWithMultiple([variableDeclaration('let', [variableDeclarator(identifier(localBinding), right)]), exportNamedDeclaration(null, [exportSpecifier(identifier(localBinding), identifier(property.name))], null)]);
  }
 ...
 */
function rewriteAsImport(path, module, firstUnsafeLocation, collectedDefaultImportNames) {
  ...
  return rewriteSingleExportRequire(path, module, collectedDefaultImportNames) || rewriteNamedExportRequire(path, module) || rewriteDeconstructedImportRequire(path, module) || rewriteSideEffectRequire(path, module);
}
 ...
 */
function rewriteSingleExportRequire(path, module, collectedDefaultImportNames) {
  var node = path.node;
  ...
  rewriteRequireAsImports('default-import', path, module, extractableDeclarations.map(function (_ref2) {
    var id = _ref2.id,
  ...
  path.replaceWithMultiple(extractableDeclarations.map(function (_ref3) {
    var id = _ref3.id,
...
function rewriteNamedExportRequire(path, module) {
  ...
  path.replaceWith(importDeclaration([importSpecifier(id, init.property)], pathNode));
...
function rewriteDeconstructedImportRequire(path, module) {
  ...
  path.replaceWith(importDeclaration(bindings.map(function (binding) {
...
function rewriteSideEffectRequire(path, module) {
  ...
  path.replaceWith(importDeclaration([], pathNode));

  return true;
}

function rewriteRequireAsImport(type, path, module, bindings, pathNode) {
  rewriteRequireAsImports(type, path, module, [{ bindings, pathNode }]);
}
  ...
  programPath.traverse({
    MemberExpression(path) {
      ...
      if (!computed && isIdentifier(object) && defaultImportNames.indexOf(object.name) !== -1 && isIdentifier(property) && property.name === 'default') {
        var dotToken = findToken('.', module.tokensInRange(object.end, path.node.end));
...
function bindingCouldBeBlockScope(binding) {
  ...
  if (binding.constantViolations.some(function (path) {
    return isVariableDeclarator(path.node);
  })) {
    return false;
  }

  var definition = binding.path;
  var definitionBlockParent = definition.findParent(function (path) {
    return path.isBlockParent();
  });
  ...
  var loopParent = definition.findParent(function (path) {
    return path.isLoop();
  });
...
function isBindingAssignedBeforeUse(binding) {
  ...
  var loopParent = binding.path.findParent(function (path) {
    return path.isLoop();
  });
  if (loopParent !== null) {
  ...
  var blockParent = binding.path.findParent(function (path) {
    return path.isBlockParent();
  });
  ...
  if (binding.constantViolations.some(function (path) {
    return path.node.end < earliestUsage && path.isAssignmentExpression() && path.parentPath.isExpressionStatement() && path.parentPath.parentPath === blockParent;
  })) {
    return true;
  }
...
function visitor$3(module) {
  ...
  return {
    ...
    AssignmentExpression(path) {
      ...
      path.replaceWith(assignmentExpression('=', objectPattern(assignments.map(function (assignment) {
...
function buildTemplateString(module, node, parts) {
  ...
  parts.forEach(function (_ref, i) {
    ...
    if (prefix || suffix || !isStringLiteral(node)) {
      ...
      quasis.push(templateElement({ cooked, raw }, false));
      cooked = '';
      raw = '';
    } else {
  ...
  quasis.push(templateElement({ cooked, raw }, true));
...
function visitor$6(module) {
  return {
    ObjectProperty(path) {
      ...
      path.replaceWith(objectMethod('method', node.key, node.value.params, node.value.body, node.computed));
    }
  };
}
...
function parseArguments(args) {
  ...
  return { input, output, blacklist, whitelist, validate, parse: parse$$1 };
}
